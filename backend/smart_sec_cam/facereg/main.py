...
from smart_sec_cam.facereg.detection import FaceDetector
...
def main(redis_url: str, redis_port: int, video_dir: str, face_threshold: int):
    ...
    # Create and start FaceDetection instance for each channel
    face_detectors = {channel: FaceDetector(channel, face_threshold=face_threshold, video_dir=video_dir)
                        for channel in active_channels}
    for detector in face_detectors.values():
        detector.run_in_background()
    while True:
        # Check for new frames from each channel and push to the corresponding FaceDetection instance
        ...
        if time.monotonic() - last_channel_check_time > CHANNEL_LIST_INTERVAL:
            ...
            for channel in active_channels:
                if channel not in face_detectors.keys():
                    ...
                    face_detectors[channel] = FaceDetector(channel,
...
if __name__ == '__main__':
    ...
    face_threshold = int(os.environ.get("FACE_THRESHOLD"))

    main(args.redis_url, args.redis_port, args.video_dir, face_threshold)
